...
THREAT_PATTERNS = {
    "english_injection": [
        ...
        (r"ignore\s+(your|the|my)\s+(instructions|rules|guidelines)", 0.70, "ignore_instructions"),
        ...
        (r"forget\s+(all\s+)?(your\s+)?(previous\s+)?instructions", 0.70, "memory_wipe"),
        (r"forget\s+(everything|all)\b.*(instructions|previous)", 0.65, "memory_wipe_flex"),
        (r"forget\s+your\s+safety\s+constraints", 0.70, "forget_safety"),
        ...
        (r"pretend\s+you\s+are\s+(a\s+)?(malicious|hacker|attacker|unrestricted)", 0.70, "pretend_malicious"),
        (r"do\s+not\s+(follow|obey)\s+(your|the|any)\s+(rules|guidelines|instructions)", 0.70, "rule_override"),
        (r"(bypass|disable|turn\s+off)\s+(your\s+)?(safety|filter|guard|restriction|security)", 0.70, "safety_bypass"),
        (r"bypass\b.*\b(safety|guidelines|restrictions|security)", 0.65, "bypass_flex"),
    ...
    "hindi_hinglish": [
        ...
        (r"(pehle|pichle)\s+ki\s+saari\s+instructions\s*(bhool|ignore)", 0.70, "hindi_forget_all"),
        (r"(bhool|ignore)\s+(jao|karo|ja)\s*(pehle|pichle|previous)?\s*(ki\s+)?(sab\s+)?(instructions)?", 0.65, "hindi_forget"),
        (r"saari\s+instructions\s*(bhool|ignore)", 0.65, "hindi_forget_instructions"),
        (r"instructions\s+(bhool\s+ja|ignore\s+karo)", 0.65, "hindi_instructions_forget"),
        ...
        (r"ab\s+se\s+tum\s+koi\s+restrictions\s+(follow\s+)?nahi", 0.65, "hindi_no_restrictions_ab"),
        (r"ab\s+se\s+tum\b", 0.45, "hindi_identity_shift"),
        (r"pehle\s+wali\s+sab\s+instructions\s*(bhool|ignore)", 0.70, "hindi_forget_all_2"),
        ...
        (r"koi\s+restrictions\s+(follow\s+)?nahi", 0.55, "hindi_no_restrictions_2"),
        (r"restrictions\s+follow\s+nahi\s+karoge", 0.55, "hindi_restrictions_not_follow"),
    ],
    "tamil_tanglish": [
        ...
        (r"adhigam\s+sollu", 0.55, "tamil_reveal_all"),
        ...
        (r"system\s+(prompt|configuration)\s*(reveal|katikai|sollu)", 0.55, "tamil_system_reveal"),
        (r"katikai\s+sollu", 0.50, "tamil_reveal"),
    ],
    "telugu": [
        (r"constraints\s+(vindinch|vindu|vittey)", 0.50, "telugu_constraints"),
        (r"intha\s+constraints\s+vindinchukoku", 0.55, "telugu_ignore_constraints"),
        (r"naa\s+kosam\s+(karyam|help)", 0.40, "telugu_help"),
        (r"nuvvu\s+(intha|anni)\s+constraints", 0.50, "telugu_you_constraints"),
    ],
}
...
def classify_threat(text: str, role: str = "guest") -> ClassifierResult:
    ...
    metadata = {
        "role": role,
        "threshold": threshold,
        "pattern_score": round(pattern_score, 4) if pattern_score is not None else 0.0,
        "semantic_score": round(semantic_score, 4) if semantic_score is not None else 0.0,
    }
